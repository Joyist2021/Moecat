...
if (get_user_class() >= UC_MODERATOR) {
    if (!$Cache->get_value('bucketpathcleanupAccess')) {
        $res = sql_query("SELECT name FROM bitbucket");////////////////////////////
        $sqlname = array();
        while ($row = mysql_fetch_array($res)) $sqlname[$row[0]] = 1;
        $dir = @opendir($bucketpath);
        $dirname = array();
        while (($file = readdir($dir)) !== false) {
            if (!preg_match('/(jpg|gif|png)$/', $file)) continue;////////////////////////////
            $dirname[$file] = 1;
            if (!$sqlname[$file]) @unlink($bucketpath . '/' . $file);////////////////////////////
        }
        closedir($dir);

        $delids = array();
        foreach (array_keys($sqlname) as $k) {
            if (!$dirname[$k]) $delids[] = sqlesc($k);
        }
        if (count($delids)) sql_query("DELETE FROM bitbucket WHERE name IN (0," . join(",", $delids) . ")");////////////////////////////
        $Cache->cache_value('bucketpathcleanupAccess', 'yes', 3600 * 24);
    }
