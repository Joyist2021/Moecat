...
$notshowfriendstags = true;
$thispagewidthscreen = false;
loggedinorreturn(true);
stdhead($lang_staff['head_staff']);//stdfoot();

$Cache->new_page('staff_page', 3600, true);
if (!$Cache->get_page()) {
    $Cache->add_whole_row();
    begin_main_frame();
    $secs = 900;
...
//--------------------- FIRST LINE SUPPORT SECTION ---------------------------//
 ...
 <td class=embedded> " . (strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg) . "</td>" .
    ...
    begin_frameindex($lang_staff['text_firstline_support'] . "<font class=small> - [<a class=altlink href=contactstaff.php><b>" . $lang_staff['text_apply_for_it'] . "</b></a>]</font>");
    ?>
...
//--------------------- film critics section ---------------------------//
 ...
 <td class=embedded> " . (strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg) . "</td>" .
    ...
    begin_frameindex($lang_staff['text_movie_critics'] . "<font class=small> - [<a class=altlink href=contactstaff.php><b>" . $lang_staff['text_apply_for_it'] . "</b></a>]</font>");
    ?>
...
//--------------------- forum moderators section ---------------------------//
 ...
 <td class=embedded> " . (strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg) . "</td>" .
    ...
    begin_frameindex($lang_staff['text_forum_moderators'] . "<font class=small> - [<a class=altlink href=contactstaff.php><b>" . $lang_staff['text_apply_for_it'] . "</b></a>]</font>");
    ?>
...
//--------------------- general staff section ---------------------------//
    ...
    while ($arr = mysql_fetch_assoc($res)) {
        if ($curr_class != $arr['class']) {
            ...
            if ($ppl != "")
                $ppl .= "<tr height=20><td class=embedded colspan=5 align=right>&nbsp;</td></tr>";
            //$ppl .= "<tr height=15><td class=embedded colspan=5 align=right>" . get_user_class_name($arr["class"],false,true,true) . "</td></tr>";
            $ppl .= "<tr>" .
                "<td class=embedded><b>" . get_user_class_name($arr["class"], false, true, true) . "</b></td>" .
 ...
 <td class=embedded> " . (strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg) . "</td>" .
            "<td class=embedded><a href=sendmessage.php?receiver=" . $arr['id'] . " title=\"" . $lang_staff['title_send_pm'] . "\">" . $sendpmimg . "</a></td>" .
            "<td class=embedded>" . $arr['stafffor'] . "</td>\n";
    }

    begin_frameindex($lang_staff['text_general_staff'] . "<font class=small> - [<a class=altlink href=contactstaff.php><b>" . $lang_staff['text_apply_for_it'] . "</b></a>]</font>");
    ?>
...
//--------------------- VIP section ---------------------------//
    ...
    unset($ppl);
    $res = sql_query("SELECT * FROM users WHERE class=" . UC_VIP . " AND status='confirmed' AND (vip_added = 'no') ORDER BY stafffor") or sqlerr();
    while ($arr = mysql_fetch_assoc($res)) {
 ...
 <td class=embedded> " . (strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg) . "</td>" .
    ...
    begin_frameindex($lang_staff['text_vip']);
    ?>
