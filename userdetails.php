...
$id = 0 + $_GET["id"];
if (!is_valid_id($id)) $id = 0 + $CURUSER[id];
int_check($id, true);
...
print("<h1 style='margin:0px'>" . get_username($user['id'], true, false, true, false, false, true) . $country . "</h1>");
...
//Xia Zuojie: Taste compatibility is extremely slow. It can takes thounsands of datebase queries. It is disabled until someone makes it fast.
        ...
        */
        ...
        $arroinvites = $Cache->get_value('arroinvites_' . $id);
        if ($arroinvites == "") {
            $relinvites = sql_query("SELECT COUNT(*) FROM users WHERE invited_by = " . mysql_real_escape_string($id)) or sqlerr(__FILE__, __LINE__);
            $arroinvites = mysql_fetch_row($relinvites);
            $Cache->cache_value('arroinvites_' . $id, $arroinvites, 3600);
        }

        tr_small("用户ID/UID", $id, 1);
        $user["inviters"] = $arroinvites[0];


        if ($user["inviters"] <= 0) ;
        //tr_small($lang_userdetails['row_invitation'], $lang_userdetails['text_no_invitation'], 1);
        else
            tr($lang_userdetails['row_invitation'], (($CURUSER[id] == $user[id] || get_user_class() >= $viewinvite_class) ? "<a href=\"invite.php?id=" . $user[id] . "\" title=\"" . $lang_userdetails['link_send_invitation'] . "\">" . $user[inviters] . "</a>" : $user[invites]), 1);
...
//}
        if (get_user_class() >= $userprofile_class OR $user["privacy"] == "low") {
            ...
            if ($user[qq]) tr_small("QQ", $user['qq'], 1);
        }
        if (get_user_class() >= $userprofile_class) {
            $iphistory = @mysql_fetch_assoc(sql_query("SELECT count(ip) as num FROM iplog WHERE userid =$id"));
            //$iphistory = mysql_num_rows($resip);

            if ($iphistory['num'] > 0)
                tr_small($lang_userdetails['row_ip_history'], $lang_userdetails['text_user_earlier_used'] . "<b><a href=\"iphistory.php?id=" . $user['id'] . "\">" . $iphistory['num'] . $lang_userdetails['text_different_ips'] . add_s($iphistory['num'], true) . "</a></b>", 1);
        ...
        if (get_user_class() >= $userprofile_class || $user["id"] == $CURUSER["id"]) {
            if ($enablelocation_tweak == 'yes') {
                ...
                $locationinfo = "[" . convertipv6($user[ip], false) . "]";
            } else $locationinfo = "";

            if ($user["id"] == $CURUSER["id"]) tr_small($lang_userdetails['row_ip_address'], "<a href='ipinfo.php' >" . $user[ip] . $locationinfo . "</a>", 1);
            else tr_small($lang_userdetails['row_ip_address'], $user[ip] . $locationinfo, 1);
        }

        $res = sql_query("SELECT agent, peer_id, ip, port FROM peers WHERE userid = $user[id] GROUP BY ip ORDER BY last_action DESC") or sqlerr();
        if (mysql_num_rows($res) > 0) {
            ...
            while ($arr = mysql_fetch_assoc($res)) {
                ...
                if (get_user_class() >= $userprofile_class || $user["id"] == $CURUSER["id"]) {
                    ...
                    if (ip2long($arr['ip'])) $clientselect .= " &nbsp;&nbsp;" . $arr["ip"] . ":" . $arr["port"] . "&nbsp;&nbsp;";
                    else $clientselect .= " &nbsp;&nbsp;[" . htmlspecialchars($arr["ip"]) . "]:" . $arr["port"] . "&nbsp;&nbsp;";
                    $clientselect .= "<span title=\"" . $loc_mod . "\">[" . $loc_pub . "]</span>";
                }
            }
        }
        ...
        if ($clientselect)
            tr_small($lang_userdetails['row_bt_client'] . "[" . ($user['MODEMAX']) . "]", $clientselect, 1);
//print $Cache->_feaKey($user["id"].'_torrent_state_userdetail_part');
        $Cache->new_page($user["id"] . '_torrent_state_userdetail_part', 3600, true);
        if (!$Cache->get_page($user["id"] == $CURUSER["id"])) {
            $Cache->add_whole_row();
            $totalimuploaded = get_row_sum("snatched", "imuploaded", "where userid=$id");
            $totalimdownloaded = get_row_sum("snatched", "imdownloaded", "where userid=$id");

            $totaltruploaded = get_row_sum("snatched", "uploaded", "where userid=$id");
            $totaltrdownloaded = get_row_sum("snatched", "downloaded", "where userid=$id");


            if ($user["downloaded"] > 0) $sr = floor($user["uploaded"] / $user["downloaded"] * 1000) / 1000;
            if ($totaltrdownloaded > 0) $sr2 = floor($totaltruploaded / $totaltrdownloaded * 1000) / 1000;
            if ($totalimdownloaded > 0) $sr3 = floor($totalimuploaded / $totalimdownloaded * 1000) / 1000;

            $sr = "<tr><td class=\"embedded\"><strong>" . $lang_userdetails['row_share_ratio'] . "</strong>(显/虚/实):  <font color=\"" . get_ratio_color($sr) . "\">" . number_format($sr, 3) . "</font> / <font color=\"" . get_ratio_color($sr3) . "\">" . number_format($sr3, 3) . "</font> / <font color=\"" . get_ratio_color($sr2) . "\">" . number_format($sr2, 3) . "</font></td></tr>";
            $xfer = "<tr><td class=\"embedded\"><strong>" . $lang_userdetails['row_uploaded'] . "</strong>(显/虚/实):  " . mksize($user["uploaded"]) . "&nbsp;/&nbsp;" . mksize($totalimuploaded) . "&nbsp;/&nbsp;" . mksize($totaltruploaded) . "</td></tr><tr><td class=\"embedded\"><strong>" . $lang_userdetails['row_downloaded'] . "</strong>(显/虚/实):  " . mksize($user["downloaded"]) . "&nbsp;/&nbsp;" . mksize($totalimdownloaded) . "&nbsp;/&nbsp;" . mksize($totaltrdownloaded) . "</td></tr>";

            tr_small($lang_userdetails['row_transfer'], "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\">" . $sr . $xfer . "</table>(虚拟,实际流量不包含删除种子)", 1);
            $Cache->end_whole_row();
            ...
            $arr3 = mysql_fetch_row($res);
            $incompletenum = $arr3[0];

            tr_small($lang_userdetails['row_uploaded_torrents'], "<a href=\"getusertorrentlist.php?userid=" . $id . "&type=uploaded\">$uploadednum</a>", 1);
            tr_small($lang_userdetails['row_current_seeding'], "<a href=\"getusertorrentlist.php?userid=" . $id . "&type=seeding\">$seedingnum</a>", 1);
            tr_small($lang_userdetails['row_current_leeching'], "<a href=\"getusertorrentlist.php?userid=" . $id . "&type=leeching\">$leechingnum</a>", 1);
            tr_small($lang_userdetails['row_completed_torrents'], "<a href=\"getusertorrentlist.php?userid=" . $id . "&type=completed\">$completednum</a>", 1);
            tr_small($lang_userdetails['row_incomplete_torrents'], "<a href=\"getusertorrentlist.php?userid=" . $id . "&type=incomplete\">$incompletenum</a>", 1);
            $Cache->end_whole_row();
    ...
    tr_small($lang_userdetails['row_internet_speed'], $download."&nbsp;&nbsp;&nbsp;&nbsp;".$upload."&nbsp;&nbsp;&nbsp;&nbsp;".$isp, 1);*/
        ...
        tr_small("院校", "<a href=\"users.php?school=$user[school]\">" . return_school_name($user["school"]) . "</a>", 1);
        ...
        if ($user["avatar"])
            tr_small($lang_userdetails['row_avatar'], return_avatar_image(htmlspecialchars(trim($user["avatar"])), "left", false), 1);
        $uclass = get_user_class_image($user["class"]);
        tr_small($lang_userdetails['row_class'], "<img alt=\"" . get_user_class_name($user["class"], false, false, true) . "\" title=\"" . get_user_class_name($user["class"], false, false, true) . "\" src=\"" . $uclass . "\" /> " . get_user_class_name($user["class"], false, true, true), 1);
        ...
        $attachmentscount = get_row_count("attachments", "where inuse=1 and userid=" . sqlesc($id));

        tr_small("附件数目", ($attachmentscount && ($user["id"] == $CURUSER["id"] || get_user_class() >= $viewhistory_class) ? "<a href=\"attachmentlog.php?userid=" . $id . "\">" . $attachmentscount . "</a>" : $attachmentscount), 1);
...
//if ($user["id"] == $CURUSER["id"] || get_user_class() >= $viewhistory_class)
        tr_small($lang_userdetails['row_karma_points'], htmlspecialchars($user[seedbonus]) . '(' . htmlspecialchars($user[seedbonusper]) . '/每小时)', 1);
...
//if ($user["ip"] && (get_user_class() >= $torrenthistory_class || $user["id"] == $CURUSER["id"]))
        {
            ...
            echo $Cache->next_row();
            /*
            tr_small($lang_userdetails['row_uploaded_torrents'], "<a href=\"javascript:getusertorrentlistajax('".$user['id']."', 'uploaded', 'a');\"><img class=\"plus\" src=\"pic/trans.gif\" id=\"pica\" alt=\"Show/Hide\" title=\"".$lang_userdetails['title_show_or_hide'] ."\" />   <u>".$lang_userdetails['text_show_or_hide']."</u></a><div id=\"ka\" style=\"display: none;\"></div>", 1);


            tr_small($lang_userdetails['row_current_seeding'], "<a href=\"javascript:getusertorrentlistajax('".$user['id']."', 'seeding', 'a1'); \"><img class=\"plus\" src=\"pic/trans.gif\" id=\"pica1\" alt=\"Show/Hide\" title=\"".$lang_userdetails['title_show_or_hide']."\" />   <u>".$lang_userdetails['text_show_or_hide']."</u></a><div id=\"ka1\" style=\"display: none;\"></div>", 1);


            tr_small($lang_userdetails['row_current_leeching'], "<a href=\"javascript:getusertorrentlistajax('".$user['id']."', 'leeching', 'a2');\"><img class=\"plus\" src=\"pic/trans.gif\" id=\"pica2\" alt=\"Show/Hide\" title=\"".$lang_userdetails['title_show_or_hide']."\" />   <u>".$lang_userdetails['text_show_or_hide']."</u></a><div id=\"ka2\" style=\"display: none;\"></div>", 1);


            tr_small($lang_userdetails['row_completed_torrents'], "<a href=\"javascript:getusertorrentlistajax('".$user['id']."', 'completed', 'a3'); \"><img class=\"plus\" src=\"pic/trans.gif\" id=\"pica3\" alt=\"Show/Hide\" title=\"".$lang_userdetails['title_show_or_hide']."\" />   <u>".$lang_userdetails['text_show_or_hide']."</u></a><div id=\"ka3\" style=\"display: none;\"></div>", 1);


            tr_small($lang_userdetails['row_incomplete_torrents'], "<a href=\"javascript:getusertorrentlistajax('".$user['id']."', 'incomplete', 'a4');\"><img class=\"plus\" src=\"pic/trans.gif\" id=\"pica4\" alt=\"Show/Hide\" title=\"".$lang_userdetails['title_show_or_hide']."\" />   <u>".$lang_userdetails['text_show_or_hide']."</u></a><div id=\"ka4\" style=\"display: none;\"></div>", 1);
            */


        }
        if ($user["info"])
            print("<tr><td class=\"rowhead nowrap\" valign=\"top\" align=\"right\">个人说明</td><td align=\"left\" class=\"text\">" . format_comment($user["info"], false) . "</td></tr>\n");
    } else {
    ...
    if ($CURUSER["id"] != $user["id"])
        if (get_user_class() >= $staffmem_class) {
            $showpmbutton = 1;
//if($user["deancheck"])tr_small("教务注册", $user["deancheck"], 1);
    ...
    if (get_user_class() >= $prfmanage_class && $user["class"] < get_user_class()) {
        begin_frameindex($lang_userdetails['text_edit_user'], true);
        ...
        if (get_user_class() >= $cruprfmanage_class) {
            ...
            tr($lang_userdetails['row_comment'], "<textarea cols=\"60\" rows=\"6\" name=\"modcomment\" readonly=\"readonly\">" . $modcomment . "</textarea>", 1);
        ...
        if (get_user_class() >= $cruprfmanage_class) {
            tr($lang_userdetails['row_change_username'], "<input type=\"text\" size=\"25\" name=\"newusername\" value=\"" . htmlspecialchars($user[username]) . "\" />", 1);
        ...
        if (get_user_class() >= $cruprfmanage_class) {
            ...
            tr($lang_userdetails['row_invites'], "<input type=\"text\" size=\"60\" name=\"invites\" value=\"" . htmlspecialchars($user[invites]) . "\" /><input type=\"checkbox\" name=\"invitesadd\" value=\"yes\">邀请码(已有" . get_row_count("invites", "WHERE inviter =" . $user[id] . " and time_invited >" . sqlesc(date("Y-m-d H:i:s", (TIMENOW + $invite_timeout * 60 * 60)))) . ")", 1);
        }
        ...
        if (get_user_class() >= $cruprfmanage_class && (!$enabled)) {
