...
elseif (strtotime($row['last_reseed']) > (TIMENOW - 3600 * 24 * 7))
    ...
    while ($row = mysql_fetch_assoc($res)) {
        $rs_subject = $lang_takereseed_target[get_user_lang($row["userid"])]['msg_reseed_request'] . '(' . $row["torrent_name"] . ')';
        ...
        sql_query("INSERT INTO messages (sender, receiver, added, subject, msg) VALUES( {$CURUSER['id']} , $row[userid], '" . date("Y-m-d H:i:s") . "'," . sqlesc($rs_subject) . ", " . sqlesc($pn_msg) . ")") or sqlerr(__FILE__, __LINE__);
    }
