...
if ($action == "edituser") {
    ...
    if (get_user_class() >= $cruprfmanage_class) {
        ...
        if ($arr['username'] != $username && check_username($username)) {
        ...
        if ($arr['invites'] != $invites && $_POST["invitesadd"] != 'yes') {
            ...
            sql_query("INSERT INTO messages (sender, receiver, subject, msg, added) VALUES(0, $userid, $subject, $msg, $added)") or sqlerr(__FILE__, __LINE__);
        } elseif ($invites && $_POST["invitesadd"] == 'yes') {
            $modcomment = date("Y-m-d") . " - Invite(有效期) amount changed from $arr[invites] to $invites by $CURUSER[username].\n" . $modcomment;
            $subject = sqlesc('邀请码变化');
            $msg = sqlesc('管理员 ' . $CURUSER[username] . ' 给你了 ' . $invites . ' 个有效期很长的邀请码');
            sql_query("INSERT INTO messages (sender, receiver, subject, msg, added) VALUES(0, $userid, $subject, $msg, $added)") or sqlerr(__FILE__, __LINE__);

            while ($invites--) {
                $hash = md5(mt_rand(1, 10000) . $CURUSER['username'] . TIMENOW . $CURUSER['passhash']);
                sql_query("INSERT INTO invites (inviter, invitee, hash, time_invited) VALUES ('" . mysql_real_escape_string($userid) . "', '', '" . mysql_real_escape_string($hash) . "', " . sqlesc('2020-01-01') . ")");
            }
    ...
    if ($warned && $curwarned != $warned) {
        ...
        if ($warnlength == 255) {
            $modcomment = date("Y-m-d") . " - Warned by " . $CURUSER['username'] . ".Reason: $warnpm.\n" . $modcomment;
            ...
            $modcomment = date("Y-m-d") . " - Warned for $dur by " . $CURUSER['username'] . ".Reason: $warnpm.\n" . $modcomment;
    ...
    $Cache->delete_value($userid . '_torrent_state_userdetail_part');
