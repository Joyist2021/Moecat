...
$userid = $CURUSER['id'];
$action = $_GET['action'];
$addsname = 0 + $_GET['sname'];
$Cache->delete_value('get_friends_row_' . $userid);
if ($addsname) {
    if (isset($_POST['addsname'])) {
        sql_query("UPDATE  friends SET sname= " . sqlesc($_POST['addsname']) . " WHERE userid=$userid AND friendid=$addsname") or sqlerr(__FILE__, __LINE__);
    } ELSE
        stderr("添加好友备注", "<form action=\"friends.php?sname=$addsname\" method=\"post\">为好友" . get_username($addsname) . "添加备注:<input type=\"text\" name=\"addsname\" size=\"64\" style=\"width: 128px; border: 1px solid gray;\" x-webkit-speech=\"\"><input type=\"submit\" value=\"确认\"></form>", false);
...
if ($action == 'add') {
    ...
    $r2 = sql_query("SELECT id FROM $table_is WHERE userid=$targetid AND $field_is=$userid") or sqlerr(__FILE__, __LINE__);
    if ($type == 'friend' && mysql_num_rows($r2) == 0) sql_query("INSERT INTO $table_is VALUES (0,$targetid, $userid , '')") or sqlerr(__FILE__, __LINE__);//双向添加
...
unset($friend_id_arr);
$res = sql_query("SELECT f.friendid as id ,f.sname,  u.last_access, u.class, u.avatar, u.title FROM friends AS f LEFT JOIN users as u ON f.friendid = u.id WHERE userid=$userid ORDER BY id") or sqlerr(__FILE__, __LINE__);
if (mysql_num_rows($res) == 0)
    ...
    while ($friend = mysql_fetch_array($res)) {
        ...
        $title = $friend["title"];
        $sname = ($friend["sname"] ? "<br /><br /><img class=\"buddylist\" src=\"pic/trans.gif\"><b>" . $friend["sname"] . "</b>" : "");
        //if (!$title)
        //$title = get_user_class_name($friend["class"],false,true,true);
        //$body1 = get_username($friend["id"]) ."<br /><br />".$lang_friends['text_last_seen_on']. gettime($friend['last_access'],true, false);
        //$body1 = get_username($friend["id"]) ." <br />$title<br /><br />".$lang_friends['text_last_seen_on']. gettime($friend['last_access'],true, false);
        $body1 = get_username($friend["id"]) . "  " . get_user_class_name($friend["class"], true, true, true) . "<br />" . $title . "<br />" . $lang_friends['text_last_seen_on'] . gettime($friend['last_access'], true, false) . $sname;
        $body2 = "
	<a href=friends.php?id=$userid&action=delete&type=friend&targetid=" . $friend['id'] . ">" . $lang_friends['text_remove_from_friends'] . "</a>" .
            "<br /><br />
	<a href=sendmessage.php?receiver=" . $friend['id'] . ">" . $lang_friends['text_send_pm'] . "</a>" .
            "<br /><br />
	<a href=friends.php?sname=" . $friend['id'] . ">备注</a>";
        ...
        print("<td class=embedded style='padding: 5px' width=30%>$body2</td></tr>\n");
        print("</table>");
        print("</td></tr>");
        print("</td></tr></table>\n");
        if ($i % 2 == 1)
