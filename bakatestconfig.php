...
<table width=700px align="center"
       style="background-image:url('pic/questionbg.png');background-position: right bottom;background-repeat: no-repeat;">
    <form action="<? echo $_SERVER['PHP_SELF'] ?>" method="post">

        <tr>
            <td class="colhead" align="center">添加测试题目</td>
        </tr>
        <tr>
            <td class="text" align="left">
                题目<input type="text" name="question" size="100" value="<? echo $editselect['question']; ?>"><input
                        type="hidden" name="questionid" value="<? echo $editselect['id']; ?>"><br/>
                选项<input type="text" name="answer1" size="90" value="<? echo $editselect['answer1']; ?>"><input
                        type="checkbox" name="answer[]"
                        value="1" <? echo(($editselectanswer / 1) % 2 == 1 ? "checked" : ""); ?>>对<br/>
                选项<input type="text" name="answer2" size="90" value="<? echo $editselect['answer2']; ?>"><input
                        type="checkbox" name="answer[]"
                        value="2" <? echo(($editselectanswer / 2) % 2 == 1 ? "checked" : ""); ?>>对<br/>
                选项<input type="text" name="answer4" size="90" value="<? echo $editselect['answer4']; ?>"><input
                        type="checkbox" name="answer[]"
                        value="4" <? echo(($editselectanswer / 4) % 2 == 1 ? "checked" : ""); ?>>对<br/>
                选项<input type="text" name="answer8" size="90" value="<? echo $editselect['answer8']; ?>"><input
                        type="checkbox" name="answer[]"
                        value="8" <? echo(($editselectanswer / 8) % 2 == 1 ? "checked" : ""); ?>>对<br/>
                提示<input type="text" name="questiontip" size="90" value="<? echo $editselect['questiontip']; ?>"><input
                        type="checkbox" name="anonymous"
                        value="yes" <? echo($editselect['anonymous'] == 'yes' ? 'checked' : ''); ?>>匿名<br/>
            </td>
        </tr>
        <tr>
            <td class="rowfollow" align="center">
                <select name="level">
                    ...
                    <? if ($MODERATOR) { ?>
                        <option value=2 <? echo($editselect['level'] == 2 ? "selected=\"selected\"" : "") ?>>老手级
                        </option>
                        <option value=1 <? echo($editselect['level'] == 1 ? "selected=\"selected\"" : "") ?>>大师级
                        </option>
                        <option value=0 <? echo($editselect && $editselect['level'] == 0 ? "selected=\"selected\"" : "") ?>>
                            新手级
                        </option>
                    <? } ?>
                </select>
                <input type="submit" value="添加"/><input type="submit" name="serchquestion" value="搜索"/></td>
        </tr>
    </form>
    <tr>
        <td class="text" align="left">
            1、BAKA&TEST系统在答题时会自动识别,因此不需要说明题目类型(单选、多选);<br/>
            2、BAKA&TEST系统自动将捣乱/秀
            <del>吉</del>
            下限者进行警告或者封号;<br/>
