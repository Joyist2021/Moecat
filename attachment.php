...
if ($_POST['tojpg'] == 'yes' && !isset($_POST['altsize'])) {
    $thumbnailtype_attachment = 'resizebigimg';
    $mustrescale = true;
}
?>
<html>
...
<table width="100%">
    ...
    if ($Attach->enable_attachment()) {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            } elseif ($filesize > $size_limit && (!$isimage || $isimage && $ext == 'gif') || $filesize >= 5242880) //do not allow file bigger than 5 MB
            {
            ...
            } else //everythins is okay
            {
                ...
                $filemd5 = md5(md5_file($file['tmp_name']) . mksecret());
                ...
                if ($isimage) //the uploaded file is a image
                {
                    ...
                    if ($imagesize) {
                        ...
                        $width = $imagesize[0];
                        $it = $imagesize[2];
                        if ($it > 3) $warning = "上传的图片格式可能为BMP";
                        if ($it != 1 || !$Attach->is_gif_ani($file['tmp_name']) || $ext != 'gif') { //if it is an animation GIF, stop creating thumbnail and adding watermark
                            if ($thumbnailtype_attachment != 'no') //create thumbnail for big image
                            {
                                ...
                                if ($filesize > $size_limit || $filesize > 1024000 * 2) {
                                    $thumbnailtype_attachment = 'resizebigimg';
                                    $mustrescale = true;
                                }
                                $newwidth = floor($width / $scale);
                                $newheight = floor($height / $scale);
                                if ($scale != 1 || $mustrescale) { //thumbnail is needed
                                    if ($it == 1)
                                    ...
                                    if ($orig && !$stop) {
                                        ...
                                        imagefilledrectangle($thumb, 0, 0, $newwidth, $newheight, $color);
                ...
                if (!$warning) //insert into database and add code to editor
                {
                    ...
                    sql_query("INSERT INTO attachments (userid, width, added, filename, filetype, filesize, location, dlkey, isimage, thumb, iszip) VALUES (" . $CURUSER['id'] . ", " . $width . ", " . sqlesc(date("Y-m-d H:i:s")) . ", " . sqlesc($origfilename) . ", " . sqlesc($filetype) . ", " . $filesize . ", " . sqlesc($db_file_location . "." . $ext) . ", " . sqlesc($dlkey) . ", " . ($isimage ? 1 : 0) . ", " . ($hasthumb ? 1 : 0) . ", " . ($iszip ? 1 : 0) . ")") or sqlerr(__FILE__, __LINE__);
                    //$count_left--;
                    echo("<script type=\"text/javascript\">parent.tag_extimage('" . "[attach]" . $dlkey . "[/attach]" . "\\n');location.replace(location.href);</script>");
                }
            }
        }
        ...
        print("<input type=\"file\" name=\"file\"" . ($count_left > 0 ? "" : " disabled=\"disabled\"") . " />&nbsp;");
        print("<input type=\"checkbox\" name=\"altsize\" value=\"yes\"" . ($altsize == 'yes' ? " checked=\"checked\"" : "") . " /><span title=\"生成" . $altthumbwidth_attachment . "X" . $altthumbheight_attachment . "略缩图且保留原图\">" . $lang_attachment['text_small_thumbnail'] . "</span>&nbsp;");
        print("<input type=\"checkbox\" name=\"tojpg\" value=\"yes\"" . ($_POST['submit'] && $_POST['tojpg'] != 'yes' ? "" : "checked=\"checked\"") . " /><span title=\"压缩图片为" . $thumbwidth_attachment . "X" . $thumbheight_attachment . "(静态图,不留原图,不统计今日剩余)\">转JPG</span>&nbsp;");
        print("<input type=\"submit\" name=\"submit\" value=\"" . $lang_attachment['submit_upload'] . "\"" . ($count_left > 0 ? "" : " disabled=\"disabled\"") . " /> ");
        if ($warning) {
