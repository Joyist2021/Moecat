...
function maketable($res, $mode = 'seeding')
{
    ...
    while ($arr = mysql_fetch_assoc($res)) {
        ...
        $sphighlight = get_torrent_bg_color($arr['sp_state'], $arr['audiocodec']);
        $sp_torrent = get_torrent_promotion_append($arr['sp_state'], $arr['audiocodec']);
...
switch ($type) {
    case 'uploaded':
    {
        $res = sql_query("SELECT torrents.id AS torrent, torrents.name as torrentname, small_descr, seeders, leechers, visible  as anonymous, categories.name AS catname, categories.image, category, sp_state,audiocodec, size, snatched.seedtime, snatched.uploaded ,snatched.imuploaded,snatched.imdownloaded FROM torrents LEFT JOIN snatched ON (torrents.id = snatched.torrentid AND snatched.userid=$id) LEFT JOIN categories ON torrents.category = categories.id WHERE torrents.owner=$id  " . (($CURUSER["id"] != $id) ? ((get_user_class() < $viewanonymous_class) ? " AND anonymous = 'no'" : "") : "") . " ORDER BY  visible DESC , torrents.added DESC") or sqlerr(__FILE__, __LINE__);
    ...
    case 'seeding':
    {
        $res = sql_query("SELECT DISTINCT(torrent),added,snatched.uploaded,snatched.downloaded,snatched.imuploaded,snatched.imdownloaded,torrents.name as torrentname, torrents.small_descr, torrents.sp_state,audiocodec, categories.name as catname,size,image,category,seeders,leechers FROM peers LEFT JOIN torrents ON peers.torrent = torrents.id LEFT JOIN categories ON torrents.category = categories.id LEFT JOIN snatched ON torrents.id = snatched.torrentid WHERE peers.userid=$id AND snatched.userid = $id AND peers.seeder='yes'  ORDER BY torrents.added DESC") or sqlerr();
        $count = mysql_num_rows($res);
        if ($count > 0) {
    ...
    case 'leeching':
    {
        $res = sql_query("SELECT DISTINCT(torrent),snatched.uploaded,snatched.downloaded,snatched.imuploaded,snatched.imdownloaded,torrents.name as torrentname, torrents.small_descr, torrents.sp_state,audiocodec, categories.name as catname,size,image,category,seeders,leechers FROM peers LEFT JOIN torrents ON peers.torrent = torrents.id LEFT JOIN categories ON torrents.category = categories.id LEFT JOIN snatched ON torrents.id = snatched.torrentid WHERE peers.userid=$id AND snatched.userid = $id AND peers.seeder='no'  ORDER BY torrents.added DESC") or sqlerr();
        $count = mysql_num_rows($res);
        if ($count > 0) {
