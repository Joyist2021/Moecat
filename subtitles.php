...
$in_detail = $_REQUEST['in_detail'];
$detail_torrent_id = $_REQUEST['detail_torrent_id'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["action"] == "upload" && ($in_detail != 'in_detail')) {
    ...
    $accept_ext = array('sub' => sub, 'srt' => srt, 'zip' => zip, 'rar' => rar, 'ssa' => ssa, 'ass' => ass, '7z' => z7z);
    ...
    */
    ...
    $id = mysql_insert_id();
    sql_query("UPDATE torrents SET subcount=subcount+1 where id = " . sqlesc($torrent_id)) or sqlerr(__FILE__, __LINE__);
    ...
    shoutbox_into('有用户为种子添加附件啦[sid' . $torrent_id . ']');
        ...
        if (mysql_num_rows($r) == 1) {
            ...
            if (get_user_class() >= $submanage_class || $a["uppedby"] == $CURUSER["id"]) {
                ...
                if ($sure == 1) {
                    ...
                    sql_query("UPDATE torrents SET subcount=subcount-1 where id = " . sqlesc($a[torrent_id])) or sqlerr(__FILE__, __LINE__);
...
if (get_user_class() >= UC_PEASANT) {
    ...
    foreach ($langs as $row) {
        $s .= "<option value=\"" . $row["id"] . "\" " . ($row["id"] == 25 ? " selected " : "") . ">" . htmlspecialchars($row["lang_name"]) . "</option>\n";
    }
    $s .= "</select></td></tr>";
...
if (get_user_class() >= UC_PEASANT) {
    ...
    print("<tr><td class=colhead>" . $lang_subtitles['col_lang'] . "</td><td width=100% class=colhead align=center>" . $lang_subtitles['col_title'] . "</td><td class=colhead align=center><img class=\"time\" src=\"pic/trans.gif\" alt=\"time\" title=\"" . $lang_subtitles['title_date_added'] . "\" /></td>
		<td class=colhead align=center><img class=\"size\" src=\"pic/trans.gif\" alt=\"size\" title=\"" . $lang_subtitles['title_size'] . "\" /></td><td class=colhead align=center>" . $lang_subtitles['col_hits'] . "</td><td class=colhead align=center>文件格式</td><td class=colhead align=center>" . $lang_subtitles['col_upped_by'] . "</td><td class=colhead align=center>" . $lang_subtitles['col_report'] . "</td></tr>\n");
    ...
    while ($arr = mysql_fetch_assoc($res)) {
        ...
        $title = "<td class=rowfollow align=left>
			<a href=\"" . ($arr['id'] <= $start_subid ? "downloadsubs_legacy.php/" . $arr['filename'] : "downloadsubs.php?torrentid=" . $arr['torrent_id'] . "&subid=" . $arr['id']) . "\"><b>" . htmlspecialchars($arr["title"]) . "</b></a>
			<a href=\"details.php?id=" . $arr['torrent_id'] . "\"><b>[种子详情]</b></a>" .
        ...
        $ext = "<td class=rowfollow align=center>" . ($arr['ext']) . "</td>\n";
        ...
        print("<tr>" . $lang . $title . $added . $size . $hits . $ext . $uppedby . $report . "</tr>\n");
        $i++;
    }
