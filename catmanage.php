...
function print_type_list($type)
{
    ...
    <div>
<span id="item" onclick="dropmenu(this);"><span style="cursor: pointer;"
                                                class="big"><b><?php echo $lang_catmanage['text_manage'] ?></b></span>
...
function print_sub_category_list($type)
{
    ...
    if (!$num)
        ...
        $res = sql_query("SELECT * FROM " . $dbtablename . " ORDER BY lid asc ,sort_index ASC " . $limit) or sqlerr(__FILE__, __LINE__);
        ?>
        <table border="1" cellspacing="0" cellpadding="5" width="940">
            <tr>
                <td class="colhead"><?php echo $lang_catmanage['col_id'] ?></td>
                <td class="colhead"><?php echo "大分类" ?></td>
            ...
            while ($row = mysql_fetch_array($res)) {
                ...
                <tr>
                    ...
                    <td class="colfollow"><a
                                href="javascript:confirm_delete('<?php echo $row['id'] ?>', '<?php echo $lang_catmanage['js_sure_to_delete_this'] ?>', 'type=<?php echo $type ?>');"><?php echo $lang_catmanage['text_delete'] ?></a>
                        |
                        <a href="?action=edit&amp;type=<?php echo $type ?>&amp;id=<?php echo $row['id'] ?>"><?php echo $lang_catmanage['text_edit'] ?></a>
                    </td>
                </tr>
                <?php
            }
...
function print_category_editor($type, $row = '')
{
    ...
    if (in_array($type, $validsubcattype))
        ...
        <div style="width: 940px">
            <h1 align="center"><a class="faqlink"
                                  href="?action=view&amp;type=<?php echo $type ?>"><?php echo $typename ?></a></h1>
            <div>
                <table border="1" cellspacing="0" cellpadding="10" width="100%">
                    ...
                    if ($type == 'searchbox') {
                        ...
                        ?>
                        <tr>
                            <td colspan="2"><?php echo $lang_catmanage['text_icon_directory_note'] ?></td>
                        </tr>
                        <?php
...
function print_sub_category_editor($type, $row = '')
{
    ...
    <div style="width: 940px">
        ...
        <table border="1" cellspacing="0" cellpadding="10" width="100%">
            ...
            tr("大分类", "<input type=\"text\" name=\"lid_index\" value=\"" . $lid_index . "\" style=\"width: 100px\" /> " . $lang_catmanage['text_lid_note'], 1);
            tr($lang_catmanage['col_order'], "<input type=\"text\" name=\"sort_index\" value=\"" . $sort_index . "\" style=\"width: 100px\" /> " . $lang_catmanage['text_order_note'], 1);
            ?>
        </table>
        <div style="text-align: center; margin-top: 10px;">
...
if ($action == 'view') {
    ...
    if (in_array($type, $validsubcattype)) {
        ...
        $lid_index = 0 + $_POST['lid_index'];
        $updateset[] = "lid=" . sqlesc($lid_index);
