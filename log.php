...
// ------------- end: stats ------------------//
	...
	<input type=hidden name=ruturnfunid value=" . $arr['id'] . ">
        ...
        case "highcharts":
            ...
            if ($wheretype == 1) {
                $searchwheretime = $TIMENOW - 3600 * 24 * 30;
                $searchwheredate = sqlesc(date('Y-m-d', $searchwheretime));
            } elseif ($wheretype == 2) {
                $searchwheretime = $TIMENOW - 3600 * 24 * 365;
                $searchwheredate = sqlesc(date('Y-m-d', $searchwheretime));
            } elseif ($wheretype == 3) {
                $searchwheretime = '0';
                $searchwheredate = sqlesc('0000-00-00');
            } else {
                $wheretype = 0;
                $searchwheretime = $TIMENOW - 3600 * 24 * 7;
                $searchwheredate = sqlesc(date('Y-m-d', $searchwheretime));
            }
            ...
            ?>
            <p><a href='?action=highcharts&type=torrent'><b>种子发布</b></a> | <a href='?action=highcharts&type=signup'><b>用户注册</b></a>
                | <a href='?action=highcharts&type=peer'><b>流量统计</b></a><a
                        href='?action=highcharts&type=mypeer&userid=<?
                        echo $CURUSER['id'] ?>'><b>(我的)</b></a> | <a href='?action=highcharts&type=com'><b>帖子回复</b></a>
                | <a href='?action=highcharts&type=bets'><b>竞猜大厅</b></a> | <a href='?action=highcharts&type=baka'><b>每日签到</b></a>
            <p>
                <a href='?action=highcharts&type=<?
                echo $_GET['type'] ?>&wheretype=0' <?
                echo($wheretype == 0 ? 'class="faqlink"' : "") ?>><b>一周</b></a> | <a href='?action=highcharts&type=<?
                echo $_GET['type'] ?>&wheretype=1' <?
                echo($wheretype == 1 ? 'class="faqlink"' : "") ?>><b>一月</b></a> | <a href='?action=highcharts&type=<?
                echo $_GET['type'] ?>&wheretype=2' <?
                echo($wheretype == 2 ? 'class="faqlink"' : "") ?>><b>一年</b></a> | <a href='?action=highcharts&type=<?
                echo $_GET['type'] ?>&wheretype=3' <?
                echo($wheretype == 3 ? 'class="faqlink"' : "") ?>><b>全部</b></a><p>
