    ...
    switch ($action) {
        ...
        case 1://更新缓存
            ...
            elseif ($movie->photo_localurl() && $movie->doubantureid()) return printnote($id . "@缓存更新成功");
                ...
                return printnote($id . "@缓存更新失败$torrentid");
            }
            break;
...
<div style="text-align: center; margin-top: 10px;">
    ...
    <table width=100%>
        <tr style="text-align: center; margin-top: 10px;">
            <td>
                <form method="post" action="imdbfind.php"><input type="hidden" name="freshall" value="1"/><input
                            type="submit" value="更新全部数据 ( <? echo $numurl[numurl] ?> )"
                            onclick="javascript:{this.disabled=true;this.form.submit()}"/></form>
            </td>
            <td>
                <form method="post" action="imdbfind.php"><input type="hidden" name="find" value="1"/><input
                            type="submit" value="更新不存在数据 ( <? echo $resnumnocache[numnocache] ?> )"
                            onclick="javascript:{this.disabled=true;this.form.submit()}"/></form>
            </td>
            <td>
                <form method="post" action="imdbfind.php"><input type="hidden" name="update" value="1"/><input
                            type="submit" value="升级豆瓣为IMDB ( <? echo $numdoubanurl[numurl] ?> )"
                            onclick="javascript:{this.disabled=true;this.form.submit()}"/></form>
            </td>
            <td>
                <form method="post" action="imdbfind.php"><input type="hidden" name="freshimg" value="1"/><input
                            type="submit" value="删除无效图片" onclick="javascript:{this.disabled=true;this.form.submit()}"/>
                </form>
            </td>
            <td>
                <form method="post" action="imdbfind.php"><input type="hidden" name="deupdate" value="1"/><input
                            type="submit" value="降级IMDB为豆瓣 ( <? echo $numimdburl[numurl] ?> )"
                            onclick="javascript:{this.disabled=true;this.form.submit()}"/></form>
            </td>
            <td>
                <form method="post" action="imdbfind.php"><input type="hidden" name="freshall" value="1"/><input
                            type="submit" value="更新全部数据" onclick="javascript:{this.disabled=true;this.form.submit()}"/>
                </form>
            </td>
        </tr>
    </table>
