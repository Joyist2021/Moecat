...
class smtp
{
    ...
    {
        $this->_connection = fsockopen($server, $port, $e, $er, 5);
        ...
        if (strpos($init, 'ESMTP') === false) {
            $this->_cmd('HELO ' . ($lhost));
        } else {
            $this->_cmd('EHLO ' . ($lhost));
        }
    }
    ...
    function from($email, $name = '')
    {
        ...
        $from = !empty($name) ? sprintf('%s <%s>', "=?UTF-8?B?" . base64_encode($name) . "?=", $email) : $email;
