    ...
    public static function decode( $data,  &$pos = 0)
    {
        ...
        if ($data[$pos] === 'd') {
            ...
            while ($data[$pos] !== 'e') {
                ...
                if (!is_string($key)) {
                    die('Invalid key type, must be string: ' . gettype($key));
                }
                $return[$key] = $value;
            }
            ksort($return);
            ...
            if ($return === '-0') {
                die('Cannot have integer value -0');
            }
            $multiplier = 1;
            if ($return[0] === '-') {
                $multiplier = -1;
            ...
            if (!ctype_digit($return)) {
                die('Cannot have non-digit values in integer number: ' . $return);
            }
        ...
        if ($start_decode) {
            if ($pos !== strlen($data)) {
                die('Could not fully decode bencode string');
            }
        }
        return $return;
    }
