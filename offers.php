...
//=== if you want to have a pending thing for uploaders use this next bit
    if (get_user_class() >= $offermanage_class && $num["allowed"] != "allowed")
        $allowedbtn = "<form method=\"post\" action=\"?id=" . $id . "&amp;allow_offer=1\"><input type=\"hidden\" value=\"" . $id . "\" name=\"offerid\" /><input class=\"btn\" type=\"submit\" value=\"" . $lang_offers['submit_allow'] . "\" />&nbsp;&nbsp;</form>";

    if (get_user_class() >= $offermanage_class && $num["allowed"] != "denied")
        $deniedbtn = "<form method=\"post\" action=\"?id=" . $id . "&amp;denied_offer=1\"><input type=\"hidden\" value=\"" . $id . "\" name=\"offerid\" /><input class=\"btn\" type=\"submit\" value=\"" . $lang_offers['text_denied'] . "\" />&nbsp;&nbsp;</form>";
    if (get_user_class() >= $offermanage_class)
        tr($lang_offers['row_allow'], "<table><tr><td class=\"embedded\">" . $allowedbtn . "</td><td class=\"embedded\">
	<form method=\"post\" action=\"?id=" . $id . "&amp;finish_offer=1\"><input type=\"hidden\" value=\"" . $id . "\" name=\"finish\" /><input class=\"btn\" type=\"submit\" value=\"" . $lang_offers['submit_let_votes_decide'] . "\" />&nbsp;&nbsp;</form>
	</td><td class=\"embedded\">" . $deniedbtn . "</td></tr></table>", 1);
...
//=== denied offer by staff
if ($_GET["denied_offer"]) {

    if (get_user_class() < $offermanage_class)
        stderr($lang_offers['std_access_denied'], $lang_offers['std_mans_job']);

    $denied_offer = 0 + $_GET["denied_offer"];
    if ($denied_offer != '1')
        stderr($lang_offers['std_error'], $lang_offers['std_smell_rat']);
    ...
    $msg = "$CURUSER[username]" . $lang_offers_target[get_user_lang($arr["userid"])]['msg_has_denied'] . "[b][url=offers.php?id=$offid&off_details=1]" . $arr[name] . "[/url][/b]. ";

    $subject = $lang_offers_target[get_user_lang($arr["userid"])]['msg_your_offer_denied'] . "(" . $arr[name] . ")";
    $deniedtime = date("Y-m-d H:i:s");
    sql_query("INSERT INTO messages (sender, receiver, added, msg, subject) VALUES(0, $arr[userid], '" . $deniedtime . "', " . sqlesc($msg) . ", " . sqlesc($subject) . ")") or sqlerr(__FILE__, __LINE__);
    sql_query("UPDATE offers SET allowed = 'denied', allowedtime = '" . $deniedtime . "' WHERE id = $offid") or sqlerr(__FILE__, __LINE__);

    write_log("$CURUSER[username] denied offer $arr[name]", 'normal');
...
if ($_GET["finish_offer"]) {
    ...
    if (!is_valid_id($offid))
        stderr($lang_offers['std_error'], $lang_offers['std_smell_rat']);
    sql_query("UPDATE offers SET allowed = 'pending'  WHERE id = $offid");
    $res = sql_query("SELECT users.username, offers.userid, offers.name FROM offers inner join users on offers.userid = users.id where offers.id = $offid") or sqlerr(__FILE__, __LINE__);
    $arr = mysql_fetch_assoc($res);
    ...
    if ($msg) {
        $subject = $lang_offers_target[get_user_lang($arr[userid])]['msg_your_offer'] . $arr[name] . $lang_offers_target[get_user_lang($arr[userid])]['msg_voted_on'] . $arr[name];
...
if ($_GET["del_offer"]) {
    ...
    elseif ($sure == 1) {
        ...
        if ($CURUSER["id"] != $num["userid"]) {
            ...
            $subject = sqlesc($lang_offers_target[get_user_lang($num["userid"])]['msg_offer_deleted'] . "(" . $arr[name] . ")");
...
$res = sql_query("SELECT DISTINCT(offers.id), offers.userid, offers.name, offers.added, offers.allowedtime, offers.comments, offers.yeah, offers.against, offers.category as cat_id, offers.allowed, categories.image, categories.name as cat , offervotes.userid as offervotesuserid FROM offers inner join categories on offers.category = categories.id  LEFT JOIN offervotes ON offers.id = offervotes.offerid and offervotes.userid =" . sqlesc($CURUSER['id']) . " $categ $search $sort $limit") or sqlerr(__FILE__, __LINE__);
...
if ($sptime == 'yes') {
    if (get_if_restricted_is_open())
        print("<li><b>" . $lang_offers['text_rule_four_three'] . "</b></li>\n");
    else
        print("<li>" . $lang_offers['text_rule_four_three'] . "</li>\n");
}
...
if (!$num)
    ...
    for ($i = 0; $i < $num; ++$i) {
        ...
        if ($arr["userid"] == $CURUSER['id'] || $arr[offervotesuserid]) {
            $VOTEROOT = "<td class=\"rowfollow nowrap\" style='padding: 5px' align=\"center\" colspan=\"3\" >" . $v_res . "</td> ";
        } else
            $VOTEROOT = "<td class=\"rowfollow nowrap\" style='padding: 5px' align=\"center\" id=\"offervotesresult" . $arr[id] . "\">" . $v_res . "</td><td class=\"rowfollow nowrap\" " . (get_user_class() < $againstoffer_class ? " colspan=\"2\" " : "") . " style='padding: 5px'>
		<a href=\"javascript: offersvoteplus(" . $arr[id] . ",'yeah');\" title=\"" . $lang_offers['title_i_want_this'] . "\"><font color=\"green\"><b>" . $lang_offers['text_yep'] . "</b></font></a>
		
		</td>" . (get_user_class() >= $againstoffer_class ? "<td class=\"rowfollow nowrap\" align=\"center\">
		
		<a href=\"javascript: offersvoteplus(" . $arr[id] . ",'against');\" title=\"" . $lang_offers['title_do_not_want_it'] . "\"><font color=\"red\"><b>" . $lang_offers['text_nah'] . "</b></font></a>
		
		</td>" : "");


        print("<tr><td class=\"rowfollow\" style=\"padding: 0px\"><a href=\"?category=" . $arr['cat_id'] . "\">" . return_category_image($arr['cat_id'], "") . "</a></td><td style='text-align: left'><a href=\"?id=" . $arr[id] . "&amp;off_details=1\" title=\"" . htmlspecialchars($arr[name]) . "\"><b>" . $allowed . htmlspecialchars($dispname) . "</b></a>" . ($CURUSER['appendnew'] != 'no' && strtotime($arr["added"]) >= $last_offer ? "<b> (<font class='new'>" . $lang_offers['text_new'] . "</font>)</b>" : "") . "</td>" . $VOTEROOT);
