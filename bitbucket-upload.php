...
$scaleh = 250; // set our height size desired
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['public'] != 'yes') {
        $filename = $CURUSER["id"] . "." . $ext;
        sql_query("DELETE FROM bitbucket WHERE name like " . sqlesc($filename)) or sqlerr(__FILE__, __LINE__);
    } else {
        $filename = $CURUSER["id"] . "_" . md5(date("YmdHis")) . "." . $ext;
    }
    $tgtfile = "$bitbucket/$filename";


    // Scale image to appropriate avatar dimensions
    $scaleh = 500; // set our height size desired
    $scalew = 300; // set our width size desired
    ...
    $url = str_replace(" ", "%20", htmlspecialchars("bitbucket/$filename"));
...
?>
    ...
    <form method="post" action=bitbucket-upload.php enctype="multipart/form-data">
        <table border=1 cellspacing=0 cellpadding=5>
            ...
            ?>
            <tr>
                <td class=rowhead><?php echo $lang_bitbucketupload['row_file'] ?></td>
                <td class="rowfollow"><input type="file" name="file" size="60"></td>
            </tr>
            <tr>
                <td colspan=2 align=middle class="toolbox">
                    <?php
                    ...
                    if (get_user_class() > UC_VIP) {
                        ...
                        <input class="checkbox" type=checkbox name=public
                               value=yes><?php echo $lang_bitbucketupload['checkbox_avatar_shared'] ?>
                    ...
                    <input type="submit" value=<?php echo $lang_bitbucketupload['submit_upload'] ?>></td>
            </tr>
        </table>
    </form>
<?php
