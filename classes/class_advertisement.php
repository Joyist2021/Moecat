...
class ADVERTISEMENT
{
    ...
    function get_mustpos()
    {
        return array('belowsearchbox', 'torrentdetail');
    }

    function get_busypos()
    {
        return array('header', 'footer', 'belownav', 'comment', 'interoverforums', 'forumpost', 'popup');
    }

    function get_ad($pos)
    {
        $mustpos = $this->get_mustpos();
        $busypos = $this->get_busypos();

        if (in_array($pos, $mustpos))
            return $this->adrow[$pos];
        elseif (in_array($pos, $busypos) && $this->showad)
            return $this->adrow[$pos];
        else return "";
    }
