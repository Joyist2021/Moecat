        ...
        switch ($action) {
            case "personal":
                if ($type == 'save') {
                    ...
                    if ($showschool = 'yes') {
                        $school = $_POST["school"];
                        if (0 + $school != 999) $updateset[] = "school = " . sqlesc($school);
                    }
                    ...
                    if ($_POST["avatar"] == '')
                        ...
                        $avatar = $_POST["avatar"];
                    $avatar = formatCodePhp2img($avatar);
                    if (preg_match("/^http:\/\/[^\s'\"<>]+\.(jpg|gif|png|jpeg)$/i", $avatar) && !preg_match("/\.php/i", $avatar) && !preg_match("/\.js/i", $avatar) && !preg_match("/\.cgi/i", $avatar)) {
                        ...
                        $avatar = htmlspecialchars(trim(str_replace((get_protocol_prefix() . "$BASEURL/"), "", $avatar)));

                        $updateset[] = "avatar = " . sqlesc($avatar);
                    }
                    $info = htmlspecialchars(trim($_POST["info"]));

                    $updateset[] = "qq = " . sqlesc($qqnum);
                ...
                stdhead($lang_usercp['head_control_panel'] . $lang_usercp['head_personal_settings']);
                ...
                $ra = sql_query("SELECT * FROM bitbucket WHERE public = '1' or owner = " . sqlesc($CURUSER["id"]) . " order by owner");
                $options = '';
...
<input type=radio name=gender" . ($CURUSER["gender"] == "Male" ? " checked" : "") . " value=Male>" . $lang_usercp['radio_male'] . "<input type=radio name=gender" . ($CURUSER["gender"] == "Female" ? " checked" : "") . " value=Female>" . $lang_usercp['radio_female'], 1);
                ...
                //School select
                if ($showschool == 'yes') {
                    $schools = "<option value=0>---- " . $lang_usercp['select_none_selected'] . " ----</option>n";
                    $schools .= "<option value=999 " . (!is_valid_id($CURUSER['school']) ? " selected" : "") . ">---- 自设 ----</option>n";
                    ...
                    tr($lang_usercp['row_school'], "<select onchange='customschool();' id=schoolselect name=school>$schools</select><input type=text name=school id=schoolname style='width: 160px' " . (!is_valid_id($CURUSER['school']) ? "value=\"$CURUSER[school]\"" : "") . " maxlength=12  disabled='disabled' />", 1);
                }
                ...
                tr("QQ", "<input type=text name=qqnum style=\"width: 400px\" value=\"" . htmlspecialchars($CURUSER["qq"]) . "\">", 1);
  ...
  $text
            ...
            case "tracker":
                ...
                if ($type == 'save') {
                    ...
                    $sbrefresh = ($_POST["sbrefresh"] ? max(60, min(3600, 0 + $_POST["sbrefresh"])) : 120);
                ...
                tr_small($lang_usercp['row_stylesheet'], "<select name=stylesheet>\n$stylesheets\n</select>&nbsp;&nbsp;<font class=small><a href=\"\addcss.php\" ><b>" . $lang_usercp['text_stylesheet_note'] . "" . $lang_usercp['text_stylesheet_link'] . "</b></a></font>.", 1);
		...
		<br /><b>" . $lang_usercp['row_torrent_page'] . ": </b><br />" . $lang_usercp['text_show'] . "<input type=text size=5 name=torrentsperpage value=" . $CURUSER['torrentsperpage'] . "> " . $lang_usercp['text_torrents_per_page'] . $lang_usercp['text_zero_equals_default'] . "<br />" .
                    ($showtooltipsetting ? "<b>" . $lang_usercp['text_tooltip_type'] . "</b>: <br />" . ($showextinfo['imdb'] == 'yes' ? "
		<input type=radio name=tooltip " . ($CURUSER['tooltip'] == 'minorimdb' ? " checked" : "") . " value=minorimdb>" . $lang_usercp['text_minor_imdb_info'] . "<br />
		<input type=radio name=tooltip " . ($CURUSER['tooltip'] == 'medianimdb' ? " checked" : "") . " value=medianimdb>" . $lang_usercp['text_median_imdb_info'] . "<br />" : "") .
                        "<input type=radio name=tooltip " . ($CURUSER['tooltip'] == 'off' ? " checked" : "") . " value=off>" . $lang_usercp['text_off'] . "<br />" : "") .
		...
		<b>" . $lang_usercp['text_comments_reviews'] . ": </b><br /><input type=checkbox name=showcomnum " . ($CURUSER['showcomnum'] == 'yes' ? " checked" : "") . " value=yes>" . $lang_usercp['text_show_comment_number']

                    //.($showtooltipsetting ? "<select name=\"showlastcom\" style=\"width: 70px;\"><option value=\"yes\" ".($CURUSER['showlastcom'] != 'no' ? " selected" : "").">".$lang_usercp['select_with']."</option><option value=\"no\" ".($CURUSER['showlastcom'] == 'no' ? " selected" : "").">".$lang_usercp['select_without']."</option></select>".$lang_usercp['text_last_comment_on_tooltip'] : "")
                    , 1);
                print("<input type=hidden name=showlastcom value=no>");
                submit();
                print("</table>");
                stdfoot();
                die;
            ...
            case "forum":
                ...
                if ($type == 'save') {
                    ...
                    $signature = htmlspecialchars(trim(str_replace((get_protocol_prefix() . "$BASEURL"), "", $_POST["signature"])));
                ...
                stdhead($lang_usercp['head_control_panel'] . $lang_usercp['head_forum_settings']);
                usercpmenu("forum");
                print ("<table border=0 cellspacing=0 cellpadding=5 width=98%>");
                form("forum");
                if ($type == 'saved')
                ...
                if ($type == 'confirm') {
                    ...
                    if ($chpassword != "") {
                        ...
                        if ($_COOKIE["c_secure_login"] == base64("yeah")) {
                            $passh = md5($CURUSER['logouttime'] . $passhash . $_SERVER["REMOTE_ADDR"]);
                            $securelogin_indentity_cookie = true;
                        } else {
                            $passh = md5($CURUSER['logouttime'] . $passhash);
                    ...
                    if ($resetlogout == 1) {
                        $updateset[] = "logouttime = " . sqlesc(TIMENOW);
                        //setcookie("logouttime", base64(TIMENOW), 0x7fffffff, "/");
                    }
                    if ($changedemail == 1) {
                        $sec = mksecret();
...
EOD;
                ...
                if ($type == 'save') {
                    ...
                    if ($resetlogout == 1)
                        print("<input type=\"hidden\" name=\"resetlogout\" value=\"1\">");
                ...
                tr_small("登出全部", "<input type=checkbox name=resetlogout value=1 />我想清除本帐号的所有自动登录记录.目前有效期自:" . date("Y-m-d H:i:s", ($CURUSER["logouttime"])) . "。<br /><font class=small>温馨提示：您可以在登录时选择自动登录的期限。</font>", 1);
...
$attachmentscount = get_row_count("attachments", "where userid=" . sqlesc($CURUSER["id"]));
//Join Date
...
?>
    <table border="0" cellspacing="0" cellpadding="5" width=98%>
        ...
        if ($enablelocation_tweak == 'yes') {
            ...
            tr_small($lang_usercp['row_ip_location'], "<a href='ipinfo.php' >" . $CURUSER["ip"] . "[" . convertipv6($CURUSER[ip], false) . "]</a>", 1);
        } else {
            tr_small($lang_usercp['row_ip_location'], $CURUSER["ip"], 1);
        }
        if ($CURUSER["avatar"])
            tr_small($lang_usercp['row_avatar'], "<img src=\"" . $CURUSER["avatar"] . "\" border=0>", 1);
        tr_small($lang_usercp['row_passkey'], get_protocol_prefix() . $announce_urls[0] . "?passkey=" . $CURUSER[passkey] . "</b><br /><br />" . get_protocol_prefix() . $announce_urls[1] . "?passkey=" . $CURUSER[passkey] . "</b></b>", 1);
        ...
        tr_small("附件数目", $attachmentscount . " [<a href=\"attachmentlog.php\">" . $lang_usercp['text_view'] . "</a>]", 1);
        if ($forumposts)
...
<?php
print("<table border=0 cellspacing=0 cellpadding=3 width=98%><tr>" .
