...
{
    global $lang_comment;
    if (!($type == "torrent" || $type == "request" || $type == "offer" || $type == "forums"))
...
if ($action == "add") {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $rasqoute = sql_query("SELECT commentpm FROM users WHERE id = $qoute_cidid") or sqlerr(__FILE__, __LINE__);
        ...
        if ($qoute_cidid && $argqoute["commentpm"] == 'yes' && $qoute_cidid != $CURUSER['id']) {
            ...
            if ($type == "torrent") {
                $notifs = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_torrent_receive_comment2'] . " [url=details.php?id=$parent_id] " . $arr['name'] . "[/url].");
                $subject = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_torrent_receive_comment2'] . "(" . $arr['name'] . ")");
            }
            if ($type == "offer") {
                $notifs = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_offer_receive_comment2'] . " [url=offers.php?id=$parent_id&off_details=1] " . $arr['name'] . "[/url].");
                $subject = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_offer_receive_comment2'] . "(" . $arr['name'] . ")");
            }
            if ($type == "request") {
                $notifs = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_request_receive_comment2'] . " [url=viewrequests.php?id=$parent_id&req_details=1] " . $arr['name'] . "[/url].");
                $subject = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_request_receive_comment2'] . "(" . $arr['name'] . ")");
            }

            $identify = sqlesc($type . $parent_id);

            //	if(!get_row_count("messages","WHERE unread ='yes'  and msg  LIKE  '%".$parent_id."%' and receiver=".$qoute_cidid )){
            //if(get_row_count("messages","WHERE  msg  LIKE  '%details%".$parent_id."%' and sender = 0 and receiver=".$qoute_cidid ))
            sql_query("delete from messages where identify  =  $identify and sender = 0 and receiver=" . $qoute_cidid);
...
//}
        $athaveowner = at_user_message($text, ($type . $parent_id), $arr['name'], $type, $arr["owner"]);
        if ($arg["commentpm"] == 'yes' && $CURUSER['id'] != $arr["owner"] && !$athaveowner && (!$qoute_cidid || $qoute_cidid && $qoute_cidid != $arr["owner"])) {
            ...
            if ($type == "offer") {
                ...
                $subject = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_offer_receive_comment'] . "(" . $arr['name'] . ")");
            }
            if ($type == "request") {
                $notifs = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_request_receive_comment'] . " [url=viewrequests.php?id=$parent_id&req_details=1] " . $arr['name'] . "[/url].");
                $subject = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_request_receive_comment'] . "(" . $arr['name'] . ")");
            }
            $identify = sqlesc($type . $parent_id);
            //	if(!get_row_count("messages","WHERE unread ='yes'  and msg  LIKE  '%".$parent_id."%' and receiver=".$arr['owner'] )){
            //if(get_row_count("messages","WHERE msg  LIKE  '%details%".$parent_id."%'  and sender = 0  and receiver=".$arr['owner'] ))
            sql_query("delete from messages where identify  =  $identify  and sender = 0 and receiver=" . $arr['owner']);
            sql_query("INSERT INTO messages (sender, receiver, subject, msg, added,identify) VALUES(0, " . $arr['owner'] . ", $subject,'', " . sqlesc(date("Y-m-d H:i:s")) . ",$identify)") or sqlerr(__FILE__, __LINE__);
...
////ADD CZY
        $res2 = sql_query("SELECT comments.text, users.username , users.id FROM comments JOIN users ON comments.user = users.id WHERE comments.id=$commentid") or sqlerr(__FILE__, __LINE__);
    ...
    //////////addCZY
    if ($sub == "quote")
        print("<input type=\"hidden\" name=\"cidid\" value=\"" . $arr2["id"] . "\"/>\n");
    begin_compose($title, ($sub == "quote" ? "quote" : "reply"), ($sub == "quote" ? htmlspecialchars("[quote=" . htmlspecialchars($arr2["username"]) . "]" . unesc(format_quotesnopic($arr2["text"])) . "[/quote]") : ""), false);
    end_compose();
    print("</form>");
    end_main_frame();
    stdfoot();
    ...
    if ($type == "torrent")
        ...
        $res = sql_query("SELECT ori_body as ori_text FROM posts  AS c  WHERE  c.id=$commentid") or sqlerr(__FILE__, __LINE__);
