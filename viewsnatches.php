...
if (0 + $_GET["all"]) {
    $countwhere = "finished = 'no' AND";
    $reswhereorder = "ORDER BY last_action DESC";
    $pagewhere = "all=1&";
} else {
    $countwhere = "finished = 'yes' AND";
    $reswhereorder = "ORDER BY last_action DESC";
    $pagewhere = "all=0&";
}


$count = get_row_count("snatched", "WHERE  $countwhere  torrentid = " . sqlesc($id));


if ($count) {
    $perpage = 25;
    list($pagertop, $pagerbottom, $limit) = pager($perpage, $count, $_SERVER["SCRIPT_NAME"] . "?id=" . htmlspecialchars($id) . "&" . $pagewhere);
    print("<p align=center>" . $lang_viewsnatches['text_users_top_finished_recently'] . "<p  align=center><a  href=\"" . $_SERVER[SCRIPT_NAME] . "?id=" . htmlspecialchars($id) . "&all=1\" ><b>查看未完成用户</b></a></p>");
    ...
    $res = sql_query("SELECT * FROM snatched WHERE $countwhere torrentid =" . sqlesc($id) . " $reswhereorder $limit");
