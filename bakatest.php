...
?>
    ...
    <form action="<? echo $_SERVER['PHP_SELF'] ?>" method="post">
        <table width="60%" border="1" cellspacing="0" cellpadding="10"
               <? if ($bakatype == 'dayper'){ ?>style="background-image:url('pic/questionbg.png');background-position: right bottom;background-repeat: no-repeat;"<?
        } ?>>
            <tr>
                <td class="text" align="left" width="100%"><?
                    print ($questinguser['num'] - count($questionallid) + 1) . "、" . $levelprint[$questing['level']] . ($type == 'checkbox' ? '[多选]' : '[单选]'); ?>
                    请问：<? echo formattest($questing["question"]); ?></td>
            </tr>
            <tr>
            ...
            ?>
            <tr>
                <td class="text" align="center" width="100%"><input type="submit" name="submit" value="提交"/><?
                    if ($canchange) {
