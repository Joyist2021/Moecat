...
dbconn();
loggedinorreturn();
stdhead("Administration", true);
print("<h1 align=center>Administration</h1>");
if (get_user_class() < UC_Warehouse) {
...
if (get_user_class() >= UC_SYSOP) {
    ...
    $query = "SELECT * FROM sysoppanel order by id";
    $sql = sql_query($query);
...
if (get_user_class() >= UC_ADMINISTRATOR) {
    ...
    print("<table width=80% border=1 cellspacing=0 cellpadding=5 align=center>");
    echo("<td class=colhead align=left>Option Name</td><td class=colhead align=left>Info</td>");
    $query = "SELECT * FROM adminpanel order by id";
    $sql = sql_query($query);
    while ($row = mysql_fetch_array($sql)) {
        $id = $row['id'];
        $name = $row['name'];
...
if (get_user_class() >= UC_MODERATOR) {
    ...
    print("<table width=80% border=1 cellspacing=0 cellpadding=5 align=center>");
    echo("<td class=colhead align=left>Option Name</td><td class=colhead align=left>Info</td>");
    $query = "SELECT * FROM modpanel order by id";
    $sql = sql_query($query);
    while ($row = mysql_fetch_array($sql)) {
        $id = $row['id'];
        $name = $row['name'];
...
///////////////////// Moderator Only \\\\\\\\\\\\\\\\\\\\\\\\\\\\
if (get_user_class() >= UC_Warehouse) {
    echo("<h1 align=center>..:: For Other Only  ::..</h1>");
    print("<br />");
    print("<br />");
    print("<table width=80% border=1 cellspacing=0 cellpadding=5 align=center>");
    echo("<td class=colhead align=left>Option Name</td><td class=colhead align=left>Info</td>");
    $query = "SELECT * FROM otherpanel order by id";
    $sql = sql_query($query);
    while ($row = mysql_fetch_array($sql)) {
        $id = $row['id'];
        $name = $row['name'];
