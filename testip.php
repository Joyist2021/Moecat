...
if ($ip) {
    ...
    if (mysql_num_rows($res) == 0)
        stderr("Result", "The IP address <b>" . ($ip) . "</b> is not banned.", false);
    else {
        ...
        while ($arr = mysql_fetch_assoc($res)) {
            $first = long2ip6($arr["first"]);
            $last = long2ip6($arr["last"]);
            $comment = ($arr["comment"]);
            $banstable .= "<tr><td>$first</td><td>$last</td><td>$comment</td></tr>\n";
        }
        $banstable .= "</table>\n";
        stderr("Result", "<table border=0 cellspacing=0 cellpadding=0><tr><td class=embedded style='padding-right: 5px'><img src=pic/smilies/excl.gif></td><td class=embedded>The IP address <b>" . ($ip) . "</b> is banned:</td></tr></table><p>" . ($banstable) . "</p>", false);
    }
}
stdhead();
