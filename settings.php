...
if ($action == 'savesettings_main')    // save main
{
    ...
    $validConfig = array('site_online', 'max_torrent_size', 'announce_interval', 'annintertwoage', 'annintertwo', 'anninterthreeage', 'anninterthree', 'signup_timeout', 'minoffervotes', 'offervotetimeout', 'offeruptimeout', 'maxsubsize', 'postsperpage', 'topicsperpage', 'torrentsperpage', 'maxnewsnum', 'max_dead_torrent_time', 'maxusers', 'maxusers2', 'torrent_dir', 'iniupload', 'SITEEMAIL', 'ACCOUNTANTID', 'ALIPAYACCOUNT', 'PAYPALACCOUNT', 'SLOGAN', 'icplicense', 'autoclean_interval_one', 'autoclean_interval_two', 'autoclean_interval_three', 'autoclean_interval_four', 'autoclean_interval_five', 'reportemail', 'invitesystem', 'registration', 'showhotmovies', 'showclassicmovies', 'showimdbinfo', 'enablenfo', 'enableschool', 'restrictemail', 'showpolls', 'showstats', 'showlastxtorrents', 'showtrackerload', 'showshoutbox', 'showfunbox', 'showoffer', 'sptime', 'showhelpbox', 'enablebitbucket', 'smalldescription', 'altname', 'extforum', 'extforumurl', 'defaultlang', 'defstylesheet', 'defstylesheet_night', 'donation', 'spsct', 'spsct2', 'spsct3', 'browsecat', 'specialcat', 'specialcat2', 'specialcat3', 'waitsystem', 'maxdlsystem', 'bitbucket', 'torrentnameprefix', 'showforumstats', 'verification', 'invite_count', 'invite_timeout', 'seeding_leeching_time_calc_start', 'startsubid', 'logo', 'showlastxforumposts');
    GetVar($validConfig);
    unset($MAIN);
...
} elseif ($action == 'savesettings_basic')    // save basic
{
    ...
    $validConfig = array('SITENAME', 'BASEURLV4', 'BASEURL', 'BASEURLV4V6', 'BASEURLV6', 'BASEHOST', 'BASEHOSTAGO', 'announce_url', 'announce_url1', 'announce_url2', 'announce_url3', 'mysql_host', 'mysql_user', 'mysql_pass', 'mysql_db', 'schoolipv6');
    GetVar($validConfig);
...
} elseif ($action == 'savesettings_tweak')    // save tweak
{
    ...
    $validConfig = array('where', 'iplog1', 'bonus', 'datefounded', 'enablelocation', 'titlekeywords', 'metakeywords', 'metadescription', 'enablesqldebug', 'sqldebug', 'cssdate', 'enabletooltip', 'prolinkimg', 'analyticscode', 'analyticscodelocation');
    GetVar($validConfig);
    unset($TWEAK);
    foreach ($validConfig as $config) {
        $TWEAK[$config] = $$config;
...
} elseif ($action == 'savesettings_attachment')    // save attachment
{
    ...
    $validConfig = array('enableattach', 'classone', 'countone', 'sizeone', 'extone', 'classtwo', 'counttwo', 'sizetwo', 'exttwo', 'classthree', 'countthree', 'sizethree', 'extthree', 'classfour', 'countfour', 'sizefour', 'extfour', 'savedirectory', 'httpdirectory', 'savedirectorytype', 'thumbnailtype', 'thumbquality', 'thumbwidth', 'thumbheight', 'watermarkpos', 'watermarkwidth', 'watermarkheight', 'watermarkquality', 'altthumbwidth', 'altthumbheight', 'wmthumb');
    GetVar($validConfig);
    unset($ATTACHMENT);
    foreach ($validConfig as $config) {
...
} elseif ($action == 'tweaksettings')        // tweak settings
{
    ...
    tr($lang_settings['row_title_keywords'], "<input type='text' style=\"width: 99%\" name='titlekeywords' value='" . ($TWEAK["titlekeywords"] ? $TWEAK["titlekeywords"] : '') . "' /> <br />" . $lang_settings['text_title_keywords_note'], 1);
    ...
    tr($lang_settings['row_meta_keywords'], "<input type='text' style=\"width: 99%\" name='metakeywords' value='" . ($TWEAK["metakeywords"] ? $TWEAK["metakeywords"] : '') . "' /> <br />" . $lang_settings['text_meta_keywords_note'], 1);
    tr($lang_settings['row_meta_description'], "<textarea cols=\"100\" style=\"width: 99%;\" rows=\"5\" name='metadescription'>" . ($TWEAK["metadescription"] ? $TWEAK["metadescription"] : '') . "</textarea> <br />" . $lang_settings['text_meta_description_note'], 1);
    tr($lang_settings['row_web_analytics_code'], "<textarea  cols=\"100\"  rows=\"20\" style=\"width: 99%;\"  name='analyticscode'>" . ($TWEAK["analyticscode"] ? $TWEAK["analyticscode"] : '') . "</textarea><br /> " . $lang_settings['text_web_analytics_code_note'], 1);

    tr("非统计代码", "<textarea  cols=\"100\"  rows=\"20\" style=\"width: 99%;\"  name='analyticscodelocation'>" . ($TWEAK["analyticscodelocation"] ? $TWEAK["analyticscodelocation"] : '') . "</textarea><br />客户端始终显示,无论用户是否能够访问外网", 1);
...
} elseif ($action == 'securitysettings')    //security settings
{
    ...
    tr($lang_settings['row_enable_image_verification'], "<input type='radio' name='iv'" . ($SECURITY["iv"] == "yes" ? " checked" : "") . " value='yes'> " . $lang_settings['text_yes'] . " <input type='radio' name='iv'" . ($SECURITY["iv"] == "no" ? " checked" : "") . " value='no'> " . $lang_settings['text_no'] . " <input type='radio' name='iv'" . ($SECURITY["iv"] == "op" ? " checked" : "") . " value='op'>电影海报<br />" . $lang_settings['text_image_verification_note'], 1);
    ...
    tr($lang_settings['row_max_ips'], "<input type='text' style=\"width: 300px\" name=maxip value='" . $SECURITY["maxip"] . "'> " . $lang_settings['text_max_ips_note'], 1);
    tr($lang_settings['row_max_login_attemps'], "<input type='text' style=\"width: 300px\" name=maxloginattempts value='" . $SECURITY["maxloginattempts"] . "'> " . $lang_settings['text_max_login_attemps_note'], 1);
...
} elseif ($action == 'authoritysettings')    //Authority settings
{
    ...
    tr($lang_settings['row_upload_special_torrent'] . "2", $lang_settings['text_minimum_class'] . classlist('uploadspecial2', $maxclass, $AUTHORITY['uploadspecial2']) . $lang_settings['text_default'] . get_user_class_name(UC_UPLOADER, false, true, true) . $lang_settings['text_upload_special_torrent_note'], 1);
    tr($lang_settings['row_upload_special_torrent'] . "3", $lang_settings['text_minimum_class'] . classlist('uploadspecial3', $maxclass, $AUTHORITY['uploadspecial3']) . $lang_settings['text_default'] . get_user_class_name(UC_UPLOADER, false, true, true) . $lang_settings['text_upload_special_torrent_note'], 1);
    ...
    tr("兑换邀请码", $lang_settings['text_minimum_class'] . classlist('exchangeinvite', $maxclass, $AUTHORITY['exchangeinvite']) . $lang_settings['text_default'] . get_user_class_name(UC_INSANE_USER, false, true, true) . "兑换手中的邀请名额为邀请码", 1);
    ...
    tr($lang_settings['row_allow_userbar'], $lang_settings['text_minimum_class'] . classlist('userbar', $maxclass, $AUTHORITY['userbar']) . $lang_settings['text_default'] . get_user_class_name(UC_MODERATOR, false, true, true) . $lang_settings['text_allow_userbar_note'], 1);
    tr($lang_settings['row_save_settings'], "<input type='submit' name='save' value='" . $lang_settings['submit_save_settings'] . "'>", 1);
    print ("</form>");
} elseif ($action == 'basicsettings')    // basic settings
{
    ...
    tr($lang_settings['row_base_url'], "<input type='text' style=\"width: 300px\" name=BASEURL value='" . ($BASIC["BASEURL"]) . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . $lang_settings['text_base_url_note'], 1);
    tr($lang_settings['row_base_url'] . "双栈", "<input type='text' style=\"width: 300px\" name=BASEURLV4V6 value='" . ($BASIC["BASEURLV4V6"]) . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . $lang_settings['text_base_url_note'], 1);
    tr($lang_settings['row_base_url'] . "V4", "<input type='text' style=\"width: 300px\" name=BASEURLV4 value='" . ($BASIC["BASEURLV4"] ? $BASIC["BASEURLV4"] : $_SERVER["HTTP_HOST"]) . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . $lang_settings['text_base_url_note'], 1);
    tr($lang_settings['row_base_url'] . "V6", "<input type='text' style=\"width: 300px\" name=BASEURLV6 value='" . ($BASIC["BASEURLV6"]) . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . $lang_settings['text_base_url_note'], 1);

    tr("根域名", "<input type='text' style=\"width: 300px\" name=BASEHOST value='" . ($BASIC["BASEHOST"] ? $BASIC["BASEHOST"] : "") . "'> " . $lang_settings['text_it_should_be'] . "如antsoul.com", 1);

    tr("曾用(其他)域名", "<input type='text' style=\"width: 300px\" name=BASEHOSTAGO value='" . ($BASIC["BASEHOSTAGO"] ? $BASIC["BASEHOSTAGO"] : "") . "'> " . $lang_settings['text_it_should_be'] . "如1.com,2.com,3.com", 1);

    tr("学校IPV6地址", "<input type='text' style=\"width: 300px\" name=schoolipv6 value='" . ($BASIC["schoolipv6"] ? $BASIC["schoolipv6"] : "") . "'> " . $lang_settings['text_it_should_be'] . "如2001:da8:6005", 1);
    ...
    tr($lang_settings['row_announce_url'], "<input type='text' style=\"width: 300px\" name=announce_url1 value='" . ($BASIC["announce_url1"] ? $BASIC["announce_url1"] : "") . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . "/announce.php", 1);
    tr($lang_settings['row_announce_url'], "<input type='text' style=\"width: 300px\" name=announce_url2 value='" . ($BASIC["announce_url2"] ? $BASIC["announce_url2"] : "") . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . "/announce.php", 1);
    tr($lang_settings['row_announce_url'], "<input type='text' style=\"width: 300px\" name=announce_url3 value='" . ($BASIC["announce_url3"] ? $BASIC["announce_url3"] : "") . "'> " . $lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"] . "/announce.php", 1);
...
} elseif ($action == 'codesettings')    // code settings
{
    ...
    tr("自定义名称颜色", $lang_settings['text_it_costs_user'] . "<input type='text' style=\"width: 50px\" name=namecolour value='" . (isset($BONUS["namecolour"]) ? $BONUS["namecolour"] : 1000) . "'>个魔力值", 1);
    tr("改名", $lang_settings['text_it_costs_user'] . "<input type='text' style=\"width: 50px\" name=namechange value='" . (isset($BONUS["namechange"]) ? $BONUS["namechange"] : 1000) . "'>个魔力值", 1);
    tr("免罪HR", $lang_settings['text_it_costs_user'] . "<input type='text' style=\"width: 50px\" name=nohr2d value='" . (isset($BONUS["nohr2d"]) ? $BONUS["nohr2d"] : 100) . "'>个魔力值", 1);
    ...
    promotion_criteria(UC_POWER_USER, "pu", 4, 32, 1.05, 0.95, 1);
    promotion_criteria(UC_ELITE_USER, "eu", 8, 256, 1.55, 1.45, 0);
    promotion_criteria(UC_CRAZY_USER, "cu", 12, 512, 2.05, 1.95, 2);
    promotion_criteria(UC_INSANE_USER, "iu", 24, 1024, 2.55, 2.45, 0);
    promotion_criteria(UC_VETERAN_USER, "vu", 24, 2048, 3.05, 2.95, 3);
    promotion_criteria(UC_EXTREME_USER, "exu", 24, 4096, 3.55, 3.45, 0);
    promotion_criteria(UC_ULTIMATE_USER, "uu", 48, 8192, 4.05, 3.95, 5);
    promotion_criteria(UC_NEXUS_MASTER, "nm", 48, 16384, 4.55, 4.45, 10);
	...
	*/
    ...
    tr($lang_settings['row_large_torrent_promotion'],
        $lang_settings['text_torrent_larger_than'] . "<input type='text' style=\"width: 50px\" name=largesize value='" . (isset($TORRENT["largesize"]) ? $TORRENT["largesize"] : 10) . "'>" . $lang_settings['text_gb_promoted_to'] . "<select name=largepro>" . promotion_selection((isset($TORRENT['largepro']) ? $TORRENT['largepro'] : 2), 1) . "</select>" . $lang_settings['text_by_system_upon_uploading'] . $lang_settings['text_large_torrent_promotion_note'] . "<br />"
        . $lang_settings['text_torrent_larger_than'] . "<input type='text' style=\"width: 50px\" name=largesize2 value='" . (isset($TORRENT["largesize2"]) ? $TORRENT["largesize2"] : 20) . "'>" . $lang_settings['text_gb_promoted_to'] . $lang_settings['select_forever'] . "<select name=largepro2>" . promotion_selection((isset($TORRENT['largepro2']) ? $TORRENT['largepro2'] : 2), 1) . "</select>" . $lang_settings['text_by_system_upon_uploading'] . $lang_settings['text_large_torrent_promotion_note'] . "<br />", 1);
...
} elseif ($action == 'mainsettings')    // main settings
{
    ...
    //初始化上传tr($lang_settings['row_initial_uploading_amount'],"<input type='text' name=iniupload style=\"width: 100px\" value=$MAIN[iniupload]> ".$lang_settings['text_initial_uploading_amount_note'], 1);
    ...
    yesorno($lang_settings['row_show_special_section'] . "2", 'spsct2', $MAIN['spsct2'], $lang_settings['text_show_special_section_note']);
    yesorno($lang_settings['row_show_special_section'] . "3", 'spsct3', $MAIN['spsct3'], $lang_settings['text_show_special_section_note']);
    ...
    tr($lang_settings['row_use_external_forum'], "<input type='radio' name='extforum'" . ($MAIN["extforum"] == "yes" ? " checked" : " checked") . " value='yes'>仅外部
	<input type='radio' name='extforum'" . ($MAIN["extforum"] == "no" ? " checked" : "") . " value='no'>仅内部
	<input type='radio' name='extforum'" . ($MAIN["extforum"] == "all" ? " checked" : "") . " value='all'>一起
	<br />", 1);
    ...
    while ($array = mysql_fetch_array($res)) {
        ...
        $scatlist2 .= "<input type=radio name=specialcat2 value='" . $array['id'] . "'" . ($MAIN["specialcat2"] == $array['id'] ? " checked" : "") . ">" . $array['name'] . "&nbsp;";
        $scatlist3 .= "<input type=radio name=specialcat3 value='" . $array['id'] . "'" . ($MAIN["specialcat3"] == $array['id'] ? " checked" : "") . ">" . $array['name'] . "&nbsp;";
    ...
    tr($lang_settings['row_special_category_mode'] . "2", $scatlist2 . "<br />" . $lang_settings['text_special_category_mode_note'], 1);
    tr($lang_settings['row_special_category_mode'] . "3", $scatlist3 . "<br />" . $lang_settings['text_special_category_mode_note'], 1);
    ...
    $res = sql_query("SELECT * FROM stylesheets ORDER BY name") or sqlerr(__FILE__, __LINE__);
    $csslist_night = "<select name=defstylesheet_night>";
    while ($array = mysql_fetch_array($res))
        $csslist_night .= "<option value='" . $array['id'] . "'" . ($MAIN["defstylesheet_night"] == $array['id'] ? " selected" : "") . ">" . $array['name'] . "</option>";
    $csslist_night .= "</select>";
    tr($lang_settings['row_default_stylesheet'], $csslist . $csslist_night . "<br />" . $lang_settings['text_default_stylesheet_note'], 1);
    ...
    tr($lang_settings['row_max_users'], "<input type='text' style=\"width: 100px\" name=maxusers value='" . ($MAIN["maxusers"] ? $MAIN["maxusers"] : "2500") . "'><input type='text' style=\"width: 100px\" name=maxusers2 value='" . ($MAIN["maxusers2"] ? $MAIN["maxusers2"] : "2500") . "'> " . $lang_settings['text_max_users'], 1);
