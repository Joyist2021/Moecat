...
?>
    ...
    <table border="1" cellspacing="0" cellpadding="5" width="940">
        <tr>
            ...
            <td class="colhead">操作
                <form action="freeleech.php" method="post">
            </td>
        </tr>
        <?php
        while ($row = mysql_fetch_array($res)) {
            ...
            <tr>
                <td class="rowfollow"
                    align='left'><?php echo htmlspecialchars($get_second_name['categories']['name'][$row['lid']] . $row['name']) ?></td>
                <td class="colfollow"><?php echo get_torrent_promotion_append(1, $row['id']) ?></td>
                <td class="colfollow"><input class=checkbox type="checkbox" name="checksecondtypeid[]"
                                             value="<? echo $row['id'] ?>"></td>
            </tr>
            <?php
        }
        ?>
        <tr>
            ...
            <td class='colhead' style='padding: 0px' align='right' colspan='3'>
                <? if (get_global_sp_state(0, 0) != 1) {
                    print "当前全局促销:" . get_torrent_promotion_append(0, 0);
                } ?>
                ...
                <select name="globalpromotiontorrent">
                    <option value="1">全局</option>
                    <option value="2">分条</option>
                </select><input type="submit" name="promotiontorrent" value="确定">
            </td>
            </form></tr>
    </table>
<?
