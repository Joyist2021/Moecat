...
$res = sql_query("SELECT * FROM users WHERE status='pending' ORDER BY id") or sqlerr();
if (mysql_num_rows($res) != 0) {
    ...
    while ($row = mysql_fetch_assoc($res)) {
        $id = $row['id'];
        list($loc_pub, $loc_mod) = get_ip_location($row['ip']);
        $addr = $loc_pub;
        ...
        print'<td align=center>&nbsp;&nbsp;&nbsp;&nbsp;' . $addr . "(" . $row['ip'] . ")" . '</td>';
        print'<td align=center>&nbsp;&nbsp;&nbsp;&nbsp;' . $row['added'] . '</td>';
        print'<td align=center><select name=confirm><option value=pending>pending</option><option value=confirmed selected>confirmed</option></select></td>';
