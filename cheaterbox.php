...
if ($torrentid && $userid) {
    $countwhere = "torrentid = " . $torrentid . " and userid = " . $userid;
    $link = "torrentid=" . $torrentid . "&userid=" . $userid;
} elseif ($userid) {
    $countwhere = "userid = " . $userid;
    $link = "userid=" . $userid;
} elseif ($torrentid) {
    $countwhere = "torrentid = " . $torrentid;
...
?>
<tr>
    <td class="colhead" colspan="12" style="text-align: right"><input type="button" value="全选"
                                                                      onClick="this.value=check(form,'全选','全不选')"><input
                type="submit" name="setdealt" value="<?php echo $lang_cheaterbox['submit_set_dealt'] ?>"/><input
