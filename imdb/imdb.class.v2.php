...
class imdb extends imdb_config
{
    ...
    function openpage($wt)
    {
        ...
        if ($this->imdbtype == 2)
            $url = "http://api.douban.com/v2/movie/subject/" . $this->imdbID . "?apikey=" . doubanapikey();
        else
            $url = "http://api.douban.com/v2/movie/imdb/tt" . $this->imdbID . "?apikey=" . doubanapikey();
    ...
    function title()
    {
        if ($this->main_title == "") {
            ...
            die($this->main_title = $this->page["Title"]->title);
        }
...
#echo $this->runtime_all();
    ...
    function plot()
    {
        if ($this->plot_plot == "") {
            ...
            $this->plot_plot = $this->page["Title"]->summary;
        }
...
#     $row_x = strpos( $temp, '<td valign="middle"' );
    ...
    {//chenzhuyu
        if ($this->doubantureid == "") {
            ...
            preg_match_all("/(http:\/\/movie\.douban\.com\/movie\/[0-9]*)/", $this->page["Title"]->id, $zh_CN);
    ...
    function creator()
    {
        if ($this->main_creator == "") {
            ...
            $this->main_creator = countarray($this->page["Title"]->author);
