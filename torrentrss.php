...
$BASEURLRSS = get_protocol_prefix() . $BASEURL . "/";
$dllink = false;
if (!$passkey = $_GET['passkey']) $passkey = $CURUSER['passkey'];
$where = "";
if ($passkey) {
    $res = sql_query("SELECT id, enabled, parked ,username FROM users WHERE passkey=" . sqlesc($passkey) . " LIMIT 1");
    ...
    elseif ($_GET['linktype'] == 'dl')
        $dllink = true;
    //IF(!isset($_GET['inclbookmarked']))die("invalid inclbookmarked");
    ...
    if ($inclbookmarked == 1) {
        ...
        if ($bookmarkarray) {
            $whereidin = implode(",", $bookmarkarray);
            $where .= ($where ? " AND " : "") . "(torrents.id IN (" . $whereidin . ") and torrents.id NOT IN( SELECT torrentid  FROM snatched WHERE userid=" . sqlesc($user['id']) . " AND finished='YES'))";
        }
    }
} else die("invalid passkey");
...
print('
	<channel>
		<title>' . addslashes($SITENAME . " " . $user['username']) . '</title>
		<link><![CDATA[' . $url . ']]></link>
		...
		<image>
			...
			<width>100</width>
			<height>100</height>
			<description>' . addslashes($SITENAME . " " . $user['username']) . '</description>
		</image>');
/*print('
