...
if ($_GET['sort'] && $_GET['type']) {
    ...
    if ($column == "owner") {
        $orderby = "case when (seeders <= 0 and pos_state = 'normal')  then users.username end   ,pos_state DESC, torrents.anonymous, users.username " . $ascdesc;
    } else {
        $orderby = "case when (seeders <= 0 and pos_state = 'normal')  then torrents." . $column . " end   ,pos_state DESC, torrents." . $column . " " . $ascdesc;
    }
    ...
    $orderby = "case when (seeders <= 0 and pos_state = 'normal')  then torrents.id end  ,pos_state DESC , torrents.id DESC";
    $pagerlink = "";
...
if (!in_array($include_dead, array(0, 1, 2)) && (get_user_class() < $seebanned_class) || (get_user_class() >= $seebanned_class) && !in_array($include_dead, array(0, 1, 2, 3, 4))) {
    $include_dead = 0;
    ...
    switch ($search_mode) {
        ...
        case 1    :
        {
            ...
            foreach ($searchstr_exploded as $searchstr_element) {
                ...
                if ($search_area == 4) $searchstr_element = (int)parse_imdb_id($searchstr_element);
        ...
        case 2    :    // exact
        {
            if ($search_area == 4) $searchstr = (int)parse_imdb_id($searchstr);
...
if ($where != "")
    $buywhere = "WHERE $where AND categories.mode = '$sectiontype'";
else $buywhere = "WHERE 1 ";

if ($allsec == 1 || $enablespecial2 != 'yes') {
    if ($where != "")
        $where = "WHERE $where ";
    else $where = "";
...
if ($count) {
    ...
    if ($allsec == 1 || $enablespecial != 'yes') {
        $query = "SELECT (torrents.id in (" . join(",", $buyid) . ")) as buy ,torrents.id, torrents.sp_state, torrents.promotion_time_type, torrents.promotion_until, torrents.banned, torrents.picktype, torrents.pos_state, torrents.category, torrents.source, torrents.medium, torrents.codec, torrents.standard, torrents.processing, torrents.team, torrents.audiocodec, torrents.leechers, torrents.seeders, torrents.name, torrents.small_descr, torrents.times_completed, torrents.size, torrents.added,torrents.last_action,torrents.comments,torrents.anonymous,torrents.owner,torrents.url,torrents.cache_stamp,torrents.rating FROM torrents " . ($search_area == 3 || $column == "owner" ? "LEFT JOIN users ON torrents.owner = users.id " : "") . " $where  $orderby2 $orderby  $limit";
    } else {
    ...
    if (!$res = $Cache->get_value('torrent_query_' . hash('md5', $query))) {
        ...
        while ($row = mysql_fetch_array($ress))
            $res[] = $row;
        $Cache->cache_value('torrent_query_' . hash('md5', $query), $res, 10);
    }
...
//$res = sql_query($query) or die(mysql_error());
    ...
    unset($res);
if (isset($searchstr))
    stdhead($lang_torrents['head_search_results_for'] . $searchstr_ori, true, "", "", true);
elseif ($sectiontype == $browsecatmode)
    stdhead($lang_torrents['head_torrents'], true, "", "", true);
else stdhead("高清区", true, "", "", true);
...
if ($allsec != 1 || $enablespecial != 'yes') { //do not print searchbox if showing bookmarked torrents from all sections;
    ...
    echo $_SERVER['PHP_SELF'] ?>">
        <table border="1" class="searchbox" cellspacing="0" cellpadding="5" width="100%">
            <tbody>
            <tr>
                <td class="colhead" align="center" colspan="2"><a href="javascript: klappe_search('searchboxmain')"><img
                                class="plus" src="pic/trans.gif" id="picsearchboxmain"
                                alt="Show/Hide"/><?php echo $lang_torrents['text_search_box'] ?><img class="plus"
                                                                                                     src="pic/trans.gif"
                                                                                                     id="pic2searchboxmain"
                                                                                                     alt="Show/Hide"/></a>
                </td>
            </tr>
            </tbody>
                            ...
                            <td class="bottom" type="hide" style="padding: 1px;padding-left: 10px">
                                <select class="med" name="incldead" style="width: 100px;">
                                    ...
                                    <?php if (get_user_class() > $seebanned_class) { ?>
                                        <option value="3"<?php print($include_dead == 3 ? " selected=\"selected\"" : ""); ?>>
                                            禁止种子
                                        </option>
                                        <option value="4"<?php print($include_dead == 4 ? " selected=\"selected\"" : ""); ?>>
                                            豆瓣链接
                                        </option>
                                    <?php } ?>
                                </select>
                                    ...
                                    <tr>
                                        <td class="embedded">
                                            <input id="searchinput" name="search" type="text"
                                                   value="<?php echo $searchstr_ori ?>" autocomplete="off"
                                                   style="width: 300px" ondblclick="suggest(event.keyCode,this.value);"
                                            ...
                                            <div id="suggcontainer"
                                                 style="text-align: left; width:100px;  display: none;">
                                                <div id="suggestions"
                                                     style="width:204px; border: 1px solid rgb(119, 119, 119); cursor: default; position: absolute; color: rgb(0,0,0); background-color: rgb(255, 255, 255);"></div>
                                            </div>
                                        </td>
                                    </tr>
                                </table>
...
if ($count) {
    ...
    elseif ($sectiontype == $specialcatmode2)
