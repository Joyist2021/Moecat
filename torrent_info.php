...
function print_array($array, $offset_symbol = "|--", $offset = "", $parent = "")
{
    ...
    switch ($array['type']) {
        case "string":
            ...
            break;
        case "integer":
            if ($parent === 'creation date') $array['value'] = date("Y-m-d H:i:s", $array['value']);
...
$Cache->new_page('torrent_info_' . $id, 3560, true);
if (!$Cache->get_page()) {
    $Cache->add_whole_row();
...
//$dict = bdec_simple($dict);
    $dict['value']['comment'] = bdec(benc_str("如果妹妹没有金发双马尾和绝对领域,即使能干,我也不要"));
// Start table
    ...
    <script type="text/javascript" language="javascript1.2">
        ...
        function clickSmack(oThisOb, oLevel, oBsID, oCol, oT) {
            ...
            if (oCol) {
                ...
                if (oThisOb.style.display != 'none') jQuery(oThisOb).slideUp(); else jQuery(oThisOb).slideDown();
            }
        }
